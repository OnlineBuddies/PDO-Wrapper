...
class mh_test extends lime_test {
    public function __construct($plan = null, $options = array()) {
        ...
        if (getenv("TEST_COVERAGE")) {
            ...
            $this->coverage->filter()->addDirectoryToWhitelist(realpath(dirname(__FILE__)."/../OLB"));
        }
        $this->test_name = $_SERVER['SCRIPT_NAME'];
    }
    ...
    private function end_coverage() {
        ...
        $f = fopen($coverageDir."/".str_replace('/', '_', $this->test_name).".cov", "w");
        fwrite($f, serialize($this->coverage));
        fclose($f);
